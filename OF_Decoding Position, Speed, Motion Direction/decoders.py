...
class GRUDecoder(object):
    ...
    def fit(self,X_train,y_train):
        ...
        model=Sequential() #Declare model
        #Add recurrent layer
        model.add(GRU(self.units,input_shape=(X_train.shape[1],X_train.shape[2]),dropout=self.dropout,recurrent_dropout=self.dropout,activation='relu')) #Within recurrent layer, include dropout
        if self.dropout!=0: model.add(Dropout(self.dropout)) #Dropout some units (recurrent layer output units)
...
class LSTMDecoder(object):
    ...
    def fit(self,X_train,y_train):
        ...
        model=Sequential() #Declare model
        #Add recurrent layer
        model.add(LSTM(self.units,input_shape=(X_train.shape[1],X_train.shape[2]),dropout=self.dropout,recurrent_dropout=self.dropout,activation='relu')) #Within recurrent layer, include dropout
...
class XGBoostDecoder(object):
    ...
    def __init__(self,max_depth=3,num_round=300,eta=0.3,colsample_bylevel=0.2,gpu=-1):
...
def load_input(inp):
    X_flat_train=inp['X_flat_train']
    X_train=inp['X_train']
    y_train=inp['y_train']    
    y_train_std=inp['y_train_std']
    y_train_mean=inp['y_train_mean']

    X_flat_test=inp['X_flat_test']
    X_test=inp['X_test']
    y_test=inp['y_test']

    position=inp['position']
    neural_data=inp['neural_data']
    
    return  X_flat_train,X_train,y_train,y_train_std,y_train_mean,X_flat_test,X_test,y_test,position,neural_data
